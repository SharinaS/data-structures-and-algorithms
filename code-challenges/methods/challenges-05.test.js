...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  let indexy = recipe.ingredients[0].indexOf('Gruffalo');
  result.push(recipe.ingredients[0].slice(indexy));

  indexy = recipe.ingredients[1].indexOf('oats');
  result.push(recipe.ingredients[1].slice(indexy));

  indexy = recipe.ingredients[2].indexOf('brown sugar');
  result.push(recipe.ingredients[2].slice(indexy));

  indexy = recipe.ingredients[3].indexOf('flour');
  result.push(recipe.ingredients[3].slice(indexy));

  indexy = recipe.ingredients[4].indexOf('pure maple syrup');
  result.push(recipe.ingredients[4].slice(indexy));

  indexy = recipe.ingredients[5].indexOf('chopped nuts');
  result.push(recipe.ingredients[5].slice(indexy));

  indexy = recipe.ingredients[6].indexOf('baking soda');
  result.push(recipe.ingredients[6].slice(indexy));

  indexy = recipe.ingredients[7].indexOf('baking powder');
  result.push(recipe.ingredients[7].slice(indexy));

  indexy = recipe.ingredients[8].indexOf('cinnamon');
  result.push(recipe.ingredients[8].slice(indexy));

  indexy = recipe.ingredients[9].indexOf('melted');
  result.push(recipe.ingredients[9].slice(indexy));

  indexy = recipe.ingredients[10].indexOf('fresh');
  result.push(recipe.ingredients[10].slice(indexy));
...
const splitFoods = (recipe) => {
  ...
  arr.forEach ( ele => {
    //make each string an array, with each word a string, take off the first two words, then combine words with a space as its own string.
    let eachArr = ele.split(' ').slice(2).join(' ');
    //console.log(eachArr);
    result.push(eachArr);
  });
  return result;
