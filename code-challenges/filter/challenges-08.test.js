...
const filterStringsWithVowels = (arr) => {
  ...
  const voweled = arr.filter( ele => ele.match(myRegex));
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const unique = arr.filter ( ele => forbiddenValues.includes(ele) === false);
  return unique;
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  let sorter = arr.filter (ele => ele.baseStat > minBaseStat);
  return sorter;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  let sorter =  arr.filter (ele => ele.baseStat > minBaseStat);
  return sorter.map( ele => ele.stat.name);
};

//-----------STOP HERE, PER NICHOLAS ---------- 
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
