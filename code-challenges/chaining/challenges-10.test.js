...
const count = (target, input) => {
  ...
  // flatten the array
  var flattened = input.reduce((accumulator, value) => {
    return accumulator.concat(value);
  },[]);
  //console.log('flattened is ', flattened);

  // count the target
  var counter = flattened.reduce((accumulator, value) => {
    if(value === target){
      accumulator +=1;
    }
    return accumulator;
...
const totalSum = (input) => {
  // Solution code here...
  const flatten = input.reduce((accumulator, value) => {
    return accumulator.concat(value);
  },[]);

  const summed = flatten.reduce((accumulator, value) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const solution = input.map (ele => {
    const removal = ele.reduce ( (answerSoFar, value) => {
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let names = data.reduce ((answerSoFar, value) => {
...
let findShortest = (data) => {
  // Solution code here...
  let shortyName;
  let shortest = data.reduce ((answerSoFar, value) =>
    (value.height > answerSoFar ? value.height : answerSoFar),
  data[0].height
  );
  console.log(shortest)

  data.forEach( ele => {
    if(ele.height === shortest){
      shortyName = ele.name;  // <------------------- not being called, so returning undefined, but works fine on Repl
    }
  });
  return shortyName;
};
