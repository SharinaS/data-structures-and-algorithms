...
Then, write a function named speaker that takes in a string and a callback function. The speaker function should return the string in all uppercase letters only by invoking the callback. <-- we're writing a callback function and calling it on a specific thing. 
------------------------------------------------------------------------------------------------ */
...
const removeWithForEach = (arr, callback) => {
  // Solution code here...
  arr.forEach( ele => { // <--- ele keeps track of element
    callback(ele, arr);
  });
  return arr;
};
...
const createList = (availableItems) => { // <-- array of current store inventory
  // Solution code here...
  const groceryList = [];
  availableItems.forEach( (ele) => {
    console.log(ele.name);
    if(ele.available === true){
      groceryList.push(ele.name);
    }
  });
  return groceryList;
};
