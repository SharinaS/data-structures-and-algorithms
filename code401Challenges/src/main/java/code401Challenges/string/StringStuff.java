...
//////////////////////////// FIRST ROUND OF WHITEBOARDING INTERVIEWS //////////////////////////////////
    ...
    // ======== Discussion of Interview Questions First Round =========
    // === Flights: Given colection of tickets, reconstruct 1-way itineray.
    // Plane Ticket has a String origin and a String destination.
    // For example:
    // SEA --> SFO,
    // SAN --> SJC,
    // SJC --> SEA
    // Answer is SAN, SJC, SEA, SFO
    // Starting ticket is the one who's origin doesn't match the other tickets.

    // === Given an array that is inorder traversal of balanced BST, make a balanced binary search tree
    // [2, 4, 5, 7, 8, 12]
    // An inorder traversal produces a sorted array.
    // The root is the middle of the array (5 or 7)
    // Set the root as a node with value as mid.
    // root.left = recursive answer from left haf of arr
    // root.right = recursive answer from right half of arr

    // if arr.length == 0: return null;
    // else if arr.length == 1: return new Node (val)
    // return new Node(arr[mid], makeTree(arr[0..mid], makeTree(arr[mid+1...])); <-- ArrayCopy is the Java form.



    // === Given a tree and a target number, see if the path weight of the leaf
    // is equivalent to the target number.  Edge case of not a leaf (don't accidently return true
    // for not a leaf).
    // Recursion makes it simpler.
    // Clever way to solve it, is to check subtracted value.

//        boolean hasWeight(Node root, int weight) {
//            if(root == null) return false;
//            if(root.left == null && root.right == null) {
//                return root.value == weight;
//            } else return hasWeight(root.left, weight - root.value) || hasWeight(root.right, weight - node.value);
//            }
//
// //////////////////////////////////////////////////////////
